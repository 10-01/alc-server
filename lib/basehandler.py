...
from external.babel import Locale
...
class BaseHandler(webapp2.RequestHandler):
    ...
    def dispatch(self):
        ...
        try:
            # csrf protection
            if self.request.method == "POST" and not self.request.path.startswith('/taskqueue') and not self.request.path.startswith('/fileupload'):
    ...
    def apikey(self):
        if self.user_id:
            return models.ApiKeys.get_apikey_by_user_id(self.user_id).key.id()
